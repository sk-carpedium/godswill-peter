...
export function usePosts(filters = {}) {
  ...
  return useQuery({
    ...
    queryFn:  () => api.entities.Post.filter({ workspace_id: workspaceId, ...filters }),
    enabled: !!workspaceId,
    staleTime: 30 * 1000,
  });
}
...
export function useTopPosts(limit = 5) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const posts = await api.entities.Post.filter({
...
export function usePostMutations() {
  ...
  const update = useMutation({
    mutationFn: ({ id, data }) => api.entities.Post.update(id, data),
  ...
  const create = useMutation({
    mutationFn: (data) => api.entities.Post.create(data),
