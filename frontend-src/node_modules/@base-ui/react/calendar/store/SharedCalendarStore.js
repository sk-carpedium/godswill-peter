...
exports.SharedCalendarStore = void 0;
 ...
 */
class SharedCalendarStore extends _store.ReactStore {
  constructor(parameters, adapter, manager, valueManager) {
    ...
    if (parameters.visibleDate) {
      ...
      initialVisibleDate = (0, _getInitialReferenceDate.getInitialReferenceDate)({
        adapter,
    ...
    this.observe(state => state.valueProp, (newValueProp, oldValueProp) => {
      if (newValueProp !== undefined && oldValueProp !== undefined && !this.state.adapter.isEqual(newValueProp, oldValueProp)) {
        ...
        if (this.state.adapter.isValid(visibleDate) && !this.state.adapter.isSameMonth(visibleDate, this.state.visibleDate)) {
          this.setVisibleDate(visibleDate, undefined, undefined, _reasons.REASONS.valuePropChange);
        }
      }
    });
  ...
  setVisibleDate = (visibleDate, nativeEvent, trigger, reason) => {
    const eventDetails = (0, _createBaseUIEventDetails.createChangeEventDetails)(reason ?? _reasons.REASONS.dayPress, nativeEvent, trigger);
  ...
  selectDate = (selectedDate, event) => {
    ...
    const referenceDate = _selectors.selectors.referenceDate(this.state);
    const activeDate = this.valueManager.getActiveDateFromValue(this.state.value) ?? referenceDate;
    const cleanSelectedDate = (0, _dateHelpers.mergeDateAndTime)(this.state.adapter, selectedDate, activeDate);
  ...
  setValue(newValue, event) {
    ...
    const eventDetails = (0, _createBaseUIEventDetails.createChangeEventDetails)(_reasons.REASONS.dayPress, event.nativeEvent, event.currentTarget, {
      getValidationError: () => this.state.manager.getValidationError(newValueWithInputTimezone, _selectors.selectors.validationProps(this.state))
    });
...
exports.SharedCalendarStore = SharedCalendarStore;