...
function getWeekList(adapter, params) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (typeof amount === 'number' && amount <= 0) {
      throw new Error(`getWeekList: The 'amount' parameter must be a positive number, but received ${amount}.`);
    }
  }
  const start = adapter.startOfWeek(date);
  const end = amount === 'end-of-month' ? adapter.endOfWeek(adapter.endOfMonth(date)) : adapter.endOfWeek(adapter.addWeeks(start, amount - 1));
  let current = start;
