...
const timezoneToRenderSelector = createSelectorMemoized(state => state.adapter, state => state.manager, state => state.value, state => state.timezoneProp, state => state.referenceDateProp, (adapter, manager, value, timezoneProp, referenceDateProp) => {
...
const validationPropsSelector = createSelectorMemoized(state => state.minDate, state => state.maxDate, (minDate, maxDate) => ({
  minDate,
  maxDate
}));
const referenceDateSelector = createSelectorMemoized(state => state.adapter, timezoneToRenderSelector, state => state.initialReferenceDateFromValue, validationPropsSelector, state => state.referenceDateProp, (adapter, timezone, initialReferenceDateFromValue, validationProps, referenceDateProp) => getInitialReferenceDate({
  adapter,
  timezone,
  validationProps,
...
const valueWithTimezoneToRenderSelector = createSelectorMemoized(timezoneToRenderSelector, state => state.manager, state => state.value, (timezone, manager, value) => manager.setTimezone(value, timezone));
const selectedDatesSelector = createSelectorMemoized(state => state.manager, state => state.value, timezoneToRenderSelector, (manager, value, timezone) => manager.getDatesFromValue(value).map(date => {
  if (manager.getTimezone(date) === timezone) {
    return date;
  }
  return manager.setTimezone(date, timezone);
}));
const isDayCellDisabledSelector = createSelector((state, value) => {
  if (state.disabled) {
    return true;
  }
...
const isDayCellUnavailableSelector = createSelector((state, value) => state.isDateUnavailable?.(value) ?? false);
const isDayButtonSelectedSelector = createSelector(state => state.adapter, selectedDatesSelector, (adapter, selectedDates, cellValue) => {
  return selectedDates.some(date => adapter.isSameDay(cellValue, date));
});
const isSetMonthButtonDisabledSelector = createSelector(state => state.adapter, validationPropsSelector, state => state.disabled, (adapter, validationProps, isCalendarDisabled, targetDate, disabledProp) => {
...
const visibleDateSelector = createSelectorMemoized(state => state.visibleDateProp, state => state.visibleDate, state => state.adapter, timezoneToRenderSelector, (visibleDateProp, visibleDate, adapter, timezone) => adapter.setTimezone(visibleDateProp ?? visibleDate, timezone));
const visibleMonthSelector = createSelectorMemoized(state => state.adapter, visibleDateSelector, (adapter, date) => adapter.startOfMonth(date));
const isValueInvalidSelector = createSelectorMemoized(state => state.manager, state => state.invalidProp, valueWithTimezoneToRenderSelector, validationPropsSelector, (manager, invalidProp, value, validationProps) => {
  if (invalidProp != null) {
    return invalidProp;
  }
...
const rootElementStateSelector = createSelectorMemoized(state => state.manager, state => state.readOnly, state => state.disabled, state => state.navigationDirection, isValueInvalidSelector, valueWithTimezoneToRenderSelector, (manager, readOnly, disabled, navigationDirection, invalid, value) => ({
...
const tabbableCellsPerMonthSelector = createSelectorMemoized(state => state.adapter, selectedDatesSelector, referenceDateSelector, (adapter, selectedDates, referenceDate) => {
...
const isDayButtonTabbableSelector = createSelector(tabbableCellsPerMonthSelector, state => state.adapter, (tabbableCellsPerMonth, adapter, date, month) => {
...
export const selectors = {
   ...
   */
  monthPageSize: createSelector(state => state.monthPageSize),
  /**
   ...
   */
  navigationDirection: createSelector(state => state.navigationDirection),
  /**
