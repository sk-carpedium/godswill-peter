...
var TabsIndicator = React5.forwardRef(function TabIndicator(componentProps, forwardedRef) {
  const {
    className,
    render,
  ...
  const activeTabPosition = React5.useMemo(() => isTabSelected ? {
    left,
    right,
    top,
    bottom
  } : null, [left, right, top, bottom, isTabSelected]);
  const activeTabSize = React5.useMemo(() => isTabSelected ? {
    width,
    height
  ...
  return (0, import_jsx_runtime2.jsxs)(React5.Fragment, {
    children: [element, isHydrating && renderBeforeHydration && (0, import_jsx_runtime2.jsx)("script", {
      nonce,
...
var TabsPanel = React6.forwardRef(function TabPanel(componentProps, forwardedRef) {
  const {
    className,
    value,
    render,
...
function useCompositeRoot(params) {
  ...
  const [internalHighlightedIndex, internalSetHighlightedIndex] = React7.useState(0);
  const isGrid = cols > 1;
  const rootRef = React7.useRef(null);
  const mergedRef = useMergedRefs(rootRef, externalRef);
  const elementsRef = React7.useRef([]);
  const hasSetDefaultIndexRef = React7.useRef(false);
...
var TabsList = React9.forwardRef(function TabsList2(componentProps, forwardedRef) {
  const {
  ...
  const [highlightedTabIndex, setHighlightedTabIndex] = React9.useState(0);
  const [tabsListElement, setTabsListElement] = React9.useState(null);
  const indicatorUpdateListenersRef = React9.useRef(/* @__PURE__ */ new Set());
  const tabResizeObserverElementsRef = React9.useRef(/* @__PURE__ */ new Set());
  const resizeObserverRef = React9.useRef(null);
  ...
  const tabsListContextValue = React9.useMemo(() => ({
    activateOnFocus,
    highlightedTabIndex,
    registerIndicatorUpdateListener,
    registerTabResizeObserverElement,
