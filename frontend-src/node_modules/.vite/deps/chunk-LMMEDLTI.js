...
var require_with_selector_development = __commonJS({
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-shim/with-selector.development.js"(exports) {
    ...
    (function() {
      ...
      var React25 = require_react(), shim = require_shim(), objectIs = "function" === typeof Object.is ? Object.is : is, useSyncExternalStore2 = shim.useSyncExternalStore, useRef18 = React25.useRef, useEffect9 = React25.useEffect, useMemo16 = React25.useMemo, useDebugValue2 = React25.useDebugValue;
...
function useFloatingPortalNode(props = {}) {
  ...
  const [containerElement, setContainerElement] = React3.useState(null);
...
var FloatingPortal = React3.forwardRef(function FloatingPortal2(componentProps, forwardedRef) {
  const {
    children,
    container,
    className,
  ...
  const focusInsideDisabledRef = React3.useRef(false);
  ...
  const portalContextValue = React3.useMemo(() => ({
    beforeOutsideRef,
    afterOutsideRef,
    beforeInsideRef,
    afterInsideRef,
...
var useFloatingParentNodeId = () => {
  var _a;
  return ((_a = React4.useContext(FloatingNodeContext)) == null ? void 0 : _a.id) || null;
};
var useFloatingTree = (externalTree) => {
  const contextTree = React4.useContext(FloatingTreeContext);
...
function FloatingFocusManager(props) {
  ...
  const orderRef = React5.useRef(["content"]);
  ...
  const pointerDownOutsideRef = React5.useRef(false);
  const lastFocusedTabbableRef = React5.useRef(null);
  const closeTypeRef = React5.useRef("");
  const lastInteractionTypeRef = React5.useRef("");
  const beforeGuardRef = React5.useRef(null);
  const afterGuardRef = React5.useRef(null);
...
function useClick(context, props = {}) {
  ...
  const pointerTypeRef = React6.useRef(void 0);
...
function useClientPoint(context, props = {}) {
  ...
  const initialRef = React7.useRef(false);
  const cleanupListenerRef = React7.useRef(null);
  const [pointerType, setPointerType] = React7.useState();
  const [reactive, setReactive] = React7.useState([]);
  ...
  const addListener = React7.useCallback(() => {
    ...
    function handleMouseMove(event) {
      ...
      if (!contains(floating, target)) {
        ...
        (_a = cleanupListenerRef.current) == null ? void 0 : _a.call(cleanupListenerRef);
...
function useDismiss(context, props = {}) {
  ...
  const pressStartedInsideRef = React8.useRef(false);
  const pressStartPreventedRef = React8.useRef(false);
  const suppressNextOutsideClickRef = React8.useRef(false);
  const {
    escapeKey: escapeKeyBubbles,
    outsidePress: outsidePressBubbles
  } = normalizeProp(bubbles);
  const touchStateRef = React8.useRef(null);
  ...
  const isComposingRef = React8.useRef(false);
  const currentPointerTypeRef = React8.useRef("");
  ...
  React8.useEffect(clearInsideReactTree, [outsidePress, clearInsideReactTree]);
...
function fastComponentRef(fn) {
  return React10.forwardRef(fastComponent(fn));
}
...
function useTriggerRegistration(id, store) {
  ...
  const registeredElementRef = React14.useRef(null);
...
function useFocus(context, props = {}) {
  ...
  const blockedReferenceRef = React15.useRef(null);
  const timeout = useTimeout();
...
function useHoverFloatingInteraction(context, parameters = {}) {
  ...
  const closeWithDelay = React16.useCallback((event) => {
...
function useHoverReferenceInteraction(context, props = {}) {
  ...
  const tree = useFloatingTree(externalTree);
  const instance = useHoverInteractionSharedState(store);
  const isHoverCloseActiveRef = React17.useRef(false);
  ...
  React17.useEffect(() => cleanupMouseMoveHandler, [cleanupMouseMoveHandler]);
  React17.useEffect(() => {
    if (!enabled) {
      return void 0;
    }
...
function useInteractions(propsList = []) {
  ...
  const getTriggerProps = React18.useCallback(
...
function useListNavigation(context, props) {
  ...
  const indexRef = React19.useRef(selectedIndex ?? -1);
  ...
  const previousMountedRef = React19.useRef(!!floatingElement);
  const previousOpenRef = React19.useRef(open);
  const forceSyncFocusRef = React19.useRef(false);
  const forceScrollIntoViewRef = React19.useRef(false);
  const cancelQueuedFocusRef = React19.useRef(null);
  ...
  const getParentOrientation = React19.useCallback(() => {
    var _a, _b, _c;
  ...
  const ariaActiveDescendantProp = React19.useMemo(() => {
...
function useRole(context, props = {}) {
  ...
  const floatingId = React20.useMemo(() => {
    var _a;
  ...
  const item = React20.useCallback(({
    active,
    selected
  }) => {
    const commonProps = {
...
function useTypeahead(context, props) {
  ...
  const prevIndexRef = React21.useRef(selectedIndex ?? activeIndex ?? -1);
...
function useFloating2(options = {}) {
  ...
  const [positionReference, setPositionReferenceRaw] = React22.useState(null);
  const domReferenceRef = React22.useRef(null);
  const tree = useFloatingTree(externalTree);
  useIsoLayoutEffect(() => {
  ...
  const setPositionReference = React22.useCallback((node) => {
  ...
  const [localDomReference, setLocalDomReference] = React22.useState(void 0);
  const [localFloatingElement, setLocalFloatingElement] = React22.useState(null);
...
use-sync-external-store/cjs/use-sync-external-store-shim/with-selector.development.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-LMMEDLTI.js.map
